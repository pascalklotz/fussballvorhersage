...
tuneGrid <- expand.grid(maxdepth = c(2,4,6)
)
...
model <- train(
  ...
  y = y_train, 
  method = "rpart2",
  trControl = control, metric = 'rounded_mae'
  , tuneGrid = tuneGrid
)
...
saveRDS(model, file = "models/my_model_goals_basic.rds")
