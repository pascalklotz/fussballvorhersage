...
saison <- "2022-2023"
filename <- "fbref_scraping/hist_2223.csv"

dat <- data.frame()
for (i in 1:nrow(teams_id)){
  datteam <- data.frame(teamname = teams_id[i,1])
  a <- paste0("https://fbref.com/en/squads/",teams_id[i,2],"/",saison, "/matchlogs/all_comps/schedule/",teams_id[i,1],"-Scores-and-Fixtures-All-Competitions")
  ...
  if(nrow(a3) > 0){
    ...
    for (j in 1:length(page)){
      a <- paste0("https://fbref.com/en/squads/",teams_id[i,2],"/",saison, "/matchlogs/all_comps/",page[j],"/",teams_id[i,1],"-Match-Logs-All-Competitions")
      a2 <- xml2::read_html(a) %>% rvest::html_nodes("#matchlogs_for") %>% rvest::html_table()
      a3 <- a2[[1]]
      colnames(a3) <- a3[1,]
      a3 <- a3[a3$Comp == "Bundesliga",]
